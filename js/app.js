    ...
    // Also persist HSC data locally (kept in sync with Firebase below)
    localStorage.setItem('fg3_hscpolicies', JSON.stringify(typeof hscPolicies !== 'undefined' ? hscPolicies : []));
    localStorage.setItem('fg3_hscmeetings', JSON.stringify(typeof hscMeetings !== 'undefined' ? hscMeetings : []));

    // Sync to Firebase (per-collection writes — safe for concurrent users)
    if (typeof database !== 'undefined') {

        // Attachments and PDFs are stored in Google Drive; RTDB keeps only the shareable URL string.
        function stripAttachments(arr) {
            return (arr || []).map(item => {
                if (!Array.isArray(item.files) || item.files.length === 0) return item;
                const stripped = item.files.map(f => {
                    ...
                    return {
                        ...
                        data: url,
                        driveId: f.driveId || extractDriveFileId(url) || undefined
                    };
                }).filter(Boolean);
                return { ...item, files: stripped };
            });
        }

        function stripHscPdfs(arr) {
            return (arr || []).map(item => {
                if (!item) return item;
                const stripped = { ...item };
                const url = resolveDriveUrl(item.pdfUrl || item.pdfStorageUrl || '');
                if (url) {
                    stripped.pdfUrl = url;
                }
                delete stripped.pdfFileData;
                delete stripped.pdfStorageUrl;
                delete stripped.pdfStoragePath;
                return stripped;
            });
        }

        // ── Per-collection writes ──────────────────────────────────────────
        // Each .update() only touches its own path. A driver save cannot
        // overwrite trucks or settings saved by another user simultaneously.
        const updates = {
            'fleetguard/drivers':     stripAttachments(drivers),
            'fleetguard/trucks':      stripAttachments(trucks),
            'fleetguard/trailers':    stripAttachments(trailers),
            'fleetguard/settings':    settings,
            'fleetguard/orders':      orders,
            'fleetguard/jobCards':    jobCards,
            'fleetguard/recycleBin':  recycleBin,
            'fleetguard/hscPolicies': stripHscPdfs(typeof hscPolicies !== 'undefined' ? hscPolicies : []),
            'fleetguard/hscMeetings': stripHscPdfs(typeof hscMeetings !== 'undefined' ? hscMeetings : []),
        ...
        database.ref('/').update(updates).catch(err => {
            ...
            if (typeof App !== 'undefined' && App.showToast) {
                App.showToast('⚠️ Cloud sync failed — changes saved locally only');
            }
        });
    }
}
                    ...
                    liveArr.forEach(item => {
                        ...
                        item.files = item.files.map(f => {
                            ...
                            if (url) {
                                ...
                                return { id: f.id, name: f.name, uploadedAt: f.uploadedAt, data: url, driveId: driveId || undefined };
                            }
...
function _afterLoad() {
    ...
    applyTheme();
            ...
            function updateSidebarBadges() {
                ...
                document.getElementById('navTruckWarn').style.display = expiredDocs ? '' : 'none';
                document.getElementById('navTruckWarn').textContent = expiredDocs;
                document.getElementById('navHighRisk').style.display = driverAlertStats.driversWithAlerts ? '' : 'none';
                document.getElementById('navHighRisk').textContent = driverAlertStats.driversWithAlerts;
                ...
                const totalAlerts = expiredDocs + expiringDriverDocs;
                document.getElementById('navAlertBadge').style.display = totalAlerts ? '' : 'none';
                document.getElementById('navAlertBadge').textContent = totalAlerts;
                ...
                purgeExpiredRecycleBin();
                const navRec = document.getElementById('navRecycleBadge');
                if (navRec) {
                    navRec.style.display = recycleBin.length ? '' : 'none';
                    navRec.textContent = recycleBin.length;
                }

                document.getElementById('sidebarStatus').textContent = `${drivers.length} drivers · ${trucks.length} trucks`;
            }
        ...
        ` : '';
            ...
            <div class="modal-body">
                ...
                <div class="section">
                    ...
                    <input id="driverAttachmentInput_${idx}" type="file" accept="image/jpeg,image/png,.jpg,.jpeg,.png,application/pdf,.pdf,video/mp4,video/quicktime,video/webm,.mp4,.mov,.webm,.m4v" multiple style="display:none" onchange="App.handleDriverFileUpload(${idx}, this.files)">
            ...
            <div class="modal-body">
                ...
                <div class="section">
                    ...
                    <input id="truckAttachmentInput_${idx}" type="file" accept="image/jpeg,image/png,.jpg,.jpeg,.png,application/pdf,.pdf" multiple style="display:none" onchange="App.handleTruckFileUpload(${idx}, this.files)">
            ...
            async function handlePolicyPdfSelect(e) {
                ...
                if (file.size > 10 * 1024 * 1024) {
                    showToast('PDF file size exceeds 10MB limit. Please select a smaller file.');
                    e.target.value = '';
                    tempPolicyPdfUrl = '';
                    tempPolicyPdfName = '';
                ...
                try {
                    ...
                    try {
                        ...
                        tempPolicyPdfUrl = dataUrl;
                        tempPolicyPdfName = file.name;
                        if (statusEl) {
                            statusEl.innerHTML = `<span style="color:var(--green)">✓ Attached locally: ${file.name} (${(file.size / 1024).toFixed(1)} KB)</span>`;
                        }
                        showToast('Stored PDF locally (Google Drive upload skipped/failed)');
                    }
                } catch (err) {
                    tempPolicyPdfUrl = '';
                    tempPolicyPdfName = '';
                ...
                <div class="modal-body" style="display:flex;flex-direction:column;gap:12px">
                    ...
                    <div>
                        ...
                        <div style="display:flex;align-items:center;gap:8px;flex-wrap:wrap">
                            ...
                            <span style="font-size:10px;color:var(--text3)">PDF, JPG, PNG accepted \u2022 Max 10MB each</span>
                        </div>
            ...
            function handleMeetingFileSelect(files) {
                ...
                for (const file of nextFiles) {
                    ...
                    if (file.size > 10 * 1024 * 1024) {
                        showToast(`"${file.name}" exceeds 10MB limit and was skipped.`);
                        continue;
                    }
            ...
            async function handleMeetingPdfSelect(e) {
                ...
                if (!file) {
                    tempPdfUrl = '';
                    tempPdfName = '';
                    if (statusEl) statusEl.textContent = 'No PDF attached (Max size 10MB)';
                    return;
                }
                if (!isPdfFile(file)) {
                    showToast('Please upload a valid PDF file');
            ...
            async function validateAttachmentFile(file) {
                if (!isSupportedAttachmentFile(file)) {
                    return { ok: false, error: `"${file.name}" is not supported. Please upload PDF, image, or video files.` };
                }
                if (isVideoFile(file)) {
                    try {
                        const duration = await getVideoDurationSeconds(file);
                        if (duration > 60) {
                            return { ok: false, error: `"${file.name}" exceeds 1 minute. Please upload a video that is 60 seconds or shorter.` };
                        }
                    } catch (_) {
                        return { ok: false, error: `Unable to validate the duration for "${file.name}".` };
                    }
                }
                return { ok: true };
            }
                ...
                return new Promise((resolve, reject) => {
                    ...
                    reader.onload = ev => {
                        ...
                        img.onload = () => {
                            ...
                            const TARGET_SIZE = 400000;
                            const MIN_QUALITY = 0.55;
                            let quality = 0.85;
                            let dataUrl = canvas.toDataURL('image/jpeg', quality);
                            while (dataUrl.length > TARGET_SIZE && quality > MIN_QUALITY) {
                                quality -= 0.05;
                            ...
                            return processFile.then(async data => {
                                ...
                                try {
                                    const result = await uploadToGoogleDrive({
                                        ...
                                        fileName: safeFileName,
                                        mimeType,
                                        folder: `fleetguard/attachments/${entityType}/${idx}`
                                    });
                                    driveUrl = result.url;
                                    driveId = result.id || extractDriveFileId(result.url) || undefined;
                ...
                loadAll(function() {
                    ...
                    if (typeof database !== 'undefined') {
                        database.ref('fleetguard').on('value', snapshot => {
                            ...
                            const data = snapshot.val();
                            if (data && typeof App.refreshFromFirebase === 'function') {
                                try { App.refreshFromFirebase(data); } catch(e) { console.warn('Sync error:', e); }
                            }
                        });
                    }
                ...
                return new Promise((resolve, reject) => {
                    ...
                    reader.onload = (e) => {
                        ...
                        img.onload = () => {
                            ...
                            const finalBytes = Math.round((bestData.length - 22) * 3 / 4);
                            resolve({ data: bestData, size: finalBytes, type: 'image/jpeg' });
                        };
                        img.onerror = reject;
...
<meta charset="UTF-8">
<meta name="viewport" content="width=device-width,initial-scale=1">
<title>Digital Job Card — ${jc.id}</title>
<style>
  body { margin: 0; background: #f1f5f9; font-family: Inter, Arial, sans-serif; color: #111; }
  .page-container { max-width: 1100px; margin: 24px auto; padding: 24px; }
  .page-header { margin-bottom: 24px; }
  .page-header h1 { margin: 0 0 4px; font-size: 26px; letter-spacing: -0.02em; }
  .page-header p { margin: 0 0 14px; color: #4b5563; }
  .toolbar { display: flex; flex-wrap: wrap; gap: 10px; margin-top: 18px; }
  .toolbar button { border: none; background: #1a237e; color: #fff; border-radius: 8px; padding: 10px 18px; font-size: 13px; font-weight: 600; cursor: pointer; }
  .toolbar button:hover { background: #0f1a49; }
  .section-label { display: inline-flex; align-items: center; padding: 10px 14px; border-radius: 999px; background: #1a237e; color: #fff; font-size: 12px; letter-spacing: 0.08em; text-transform: uppercase; margin-bottom: 16px; }
  @media print { .toolbar { display: none !important; } .page-container { padding: 0; } }
</style>
</head>
<body>
<div class="page-container">
  <div class="page-header">
    ...
    <h1>${jc.id}</h1>
    <p>${formatTruckLabelFromPlate(jc.plate)} · ${xmlEscape(jc.driver)}</p>
    <div class="toolbar">
  ...
  <section class="full-section">
    ...
    ${proofSection}
            ...
            function generateJobCardReportHtml(jc) {
                ...
                return `<div style="font-family:'Segoe UI',Helvetica,Arial,sans-serif;width:100%;max-width:800px;margin:0 auto;padding:0;background:#fff;color:#0f172a;box-sizing:border-box">
                    <div style="border-bottom:2px solid #1e293b;padding-bottom:10px;margin-bottom:14px;text-align:center">
                        <h1 style="margin:0 0 4px 0;font-size:20px;font-weight:800;letter-spacing:0.02em;color:#0f172a">3 RAG COMPANY LTD — JOB CARD REPORT</h1>
                        <p style="margin:0;font-size:12px;color:#475569;font-weight:600">${xmlEscape(jc.id)} &nbsp;·&nbsp; ${formatTruckLabelFromPlate(jc.plate)} &nbsp;·&nbsp; Approved: ${jcApprovalTimestamp(jc).slice(0, 10)}</p>
                    </div>
                    <div style="display:flex;justify-content:space-between;flex-wrap:wrap;gap:10px;background:#f8fafc;padding:10px 14px;border:1px solid #e2e8f0;border-radius:6px;margin-bottom:14px;font-size:12px">
                        <div><strong>Truck:</strong> ${formatTruckLabelFromPlate(jc.plate)} ${trk?.model ? '· ' + xmlEscape(trk.model) : ''} &nbsp;·&nbsp; <strong>Health:</strong> ${trk ? healthScore(trk) + '%' : 'N/A'}</div>
                        <div><strong>Driver:</strong> ${xmlEscape(jc.driver)} &nbsp;·&nbsp; <strong>Approved By:</strong> ${xmlEscape(jc.approvedBy || 'Supervisor')}</div>
                    </div>
                    ...
                    <table style="width:100%;border-collapse:collapse;margin-bottom:14px">
                        ...
                        <tbody>${driverRows || '<tr><td colspan="2" style="padding:8px;text-align:center;color:#94a3b8">— No driver lines —</td></tr>'}</tbody>
                    </table>
                    ...
                    <table style="width:100%;border-collapse:collapse;margin-bottom:16px">
                        ...
                        <tbody>
                            ${mechRows || '<tr><td colspan="6" style="padding:8px;text-align:center;color:#94a3b8">— No completed lines —</td></tr>'}
                    ...
                    <div style="display:grid;grid-template-columns:1fr 1fr 1fr;gap:20px;margin-top:24px;text-align:center;font-size:11px;color:#475569">
            ...
            function generateJobCardReport(jc) {
                if (!jc) return;
                const html = '<!DOCTYPE html><html><head><title>' + jc.id + '</title></head><body>' + generateJobCardReportHtml(jc) + '</body></html>';
                const win = window.open('', '_blank');
