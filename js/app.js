        ...
        function saveAll() {
            ...
            function restoreFromRecycleBin(itemId) {
                ...
                switch (item.type) {
                    ...
                    case 'accident': {
                        let driver = drivers.find(d => d._idx === item.meta?.driverIdx);
                        if (!driver && item.meta?.driverName) {
                            driver = drivers.find(d => d.name === item.meta.driverName);
                        }
                        if (driver) {
                            if (!Array.isArray(driver.accidentsList)) driver.accidentsList = [];
                            driver.accidentsList.push(restored);
                        } else {
                            showToast('⚠ Driver not found — record could not be restored');
                            return;
                        }
                        break;
                    }
            ...
            function _recycleTypeInfo(type) {
                const map = {
                    ...
                    accident:  { icon: '🚗', label: 'Accident / Incident', color: '#a78bfa' }
                };
            ...
            function renderRecycleBin() {
                ...
                const counts = { driver: 0, truck: 0, trailer: 0, order: 0, jobcard: 0, violation: 0, accident: 0 };
                ...
                if (typeof database !== 'undefined') {
                    database.ref('fleetguard').once('value').then(snapshot => {
                        ...
                        if (data) {
                            ...
                            // Firebase empty — fall back to localStorage (offline mode)
...
function _afterLoad() {
    ...
    drivers.forEach(drv => {
        ...
        if (drv.license_plate && drv.assignmentHistory.length === 0) {
            const assignedTruckObj = trucks.find(t => String(t.plate || '').trim().toUpperCase() === String(drv.license_plate || '').trim().toUpperCase());
            const trId = (drv.custom && drv.custom.Trailer) || (assignedTruckObj ? assignedTruckObj.trailer : '') || '';
    ...
    trucks.forEach(trk => {
        ...
        if (!Array.isArray(trk.assignmentHistory)) trk.assignmentHistory = [];
        if (trk.assignmentHistory.length === 0) {
            const curD = drivers.find(d => String(d.license_plate || '').trim().toUpperCase() === String(trk.plate || '').trim().toUpperCase());
            if (curD || trk.trailer) {
                trk.assignmentHistory.push({
                    driverName: curD ? curD.name : 'Unassigned',
                    driverId: curD ? curD.id : '',
                    trailer: trk.trailer || '',
                    action: 'Initial Assignment',
                    date: (curD && curD.hire_date) || 'Initial Assignment',
                    notes: 'Initial truck assignment recorded'
                });
            }
        }
    });
    ...
    applyTheme();
            ...
            function handleDrawerTruckSearch(val) {
                ...
                const exact = trucks.find(t => String(t.plate || '').trim().toUpperCase() === clean);
                if (exact) {
                    selectedDrawerTruckPlate = exact.plate;
                } else {
                    const drvExact = (drivers || []).find(d => String(d.name || '').trim().toUpperCase() === clean && d.license_plate);
                    if (drvExact) {
                        const trkMatch = trucks.find(t => String(t.plate || '').trim().toUpperCase() === drvExact.license_plate.toUpperCase());
                        if (trkMatch) {
                            selectedDrawerTruckPlate = trkMatch.plate;
                        }
                    }
                }
                renderTruckAssignmentDrawer();
            ...
            function renderTruckAssignmentDrawer() {
                ...
                const filteredTrucks = searchTerm ? sortedTrucks.filter(t => {
                    ...
                    const drvName = (td && td.name ? td.name : '').toLowerCase();
                    return plate.includes(searchTerm) || brand.includes(searchTerm) || model.includes(searchTerm) || drvName.includes(searchTerm);
                }) : sortedTrucks;
            ...
            function recordDriverTruckAssignment(driver, truckPlate, assignmentDate, trailer, action, notes) {
                if (!driver) return;
                ...
                const dateStr = assignmentDate || new Date().toISOString().slice(0, 10);
                const trId = trailer !== undefined ? String(trailer || '').trim() : (cleanPlate !== 'Unassigned' ? (getTruckByPlate(cleanPlate) || {}).trailer || '' : '');
                const act = action || (cleanPlate === 'Unassigned' ? 'Unassigned' : 'Reassignment');
            ...
            function recordTruckAssignment(truck, driverName, driverId, trailer, assignmentDate, action, notes) {
                if (!truck) return;
                if (!Array.isArray(truck.assignmentHistory)) truck.assignmentHistory = [];
                const cleanDriver = String(driverName || '').trim() || 'Unassigned';
                const cleanId = String(driverId || '').trim();
                const cleanTrailer = trailer !== undefined ? String(trailer || '').trim() : String(truck.trailer || '').trim();
                const dateStr = assignmentDate || new Date().toISOString().slice(0, 10);
                const act = action || (cleanDriver === 'Unassigned' ? 'Released' : 'Driver Reassigned');
                
                truck.assignmentHistory.push({
            ...
            function confirmTruckAssignment() {
                ...
                const prevTruck = prevTruckPlate ? trucks.find(t => String(t.plate).trim().toUpperCase() === prevTruckPlate.toUpperCase()) : null;
                ...
                if (currentTruckDriver) {
                    if (isSwapAssignment) {
                        ...
                        recordDriverTruckAssignment(driver, selectedDrawerTruckPlate, todayStr, truck.trailer || '', 'Swap', `Swapped trucks with ${currentTruckDriver.name} (${currentTruckDriver.id})`);
                        recordTruckAssignment(truck, driver.name, driver.id, truck.trailer || '', todayStr, 'Swap', `Swapped driver from ${currentTruckDriver.name} to ${driver.name}`);
                        syncDriverIdWithTruck(driver);

                        currentTruckDriver.license_plate = prevTruckPlate;
                        recordDriverTruckAssignment(currentTruckDriver, prevTruckPlate, todayStr, prevTruck ? prevTruck.trailer || '' : '', 'Swap', `Swapped trucks with ${driver.name} (${driver.id}) to ${prevTruckPlate || 'None'}`);
                        if (prevTruck) {
                            recordTruckAssignment(prevTruck, currentTruckDriver.name, currentTruckDriver.id, prevTruck.trailer || '', todayStr, 'Swap', `Swapped driver from ${driver.name} to ${currentTruckDriver.name}`);
                        }
                        syncDriverIdWithTruck(currentTruckDriver);

                        const prevDesc = prevTruckPlate ? prevTruckPlate : "None";
                        detailsLog = `Swapped trucks: Assigned ${driver.name} to ${selectedDrawerTruckPlate} (trailer: ${truck.trailer || '-'}) and ${currentTruckDriver.name} to ${prevDesc}.`;
                    } else {
                        driver.license_plate = selectedDrawerTruckPlate;
                        recordDriverTruckAssignment(driver, selectedDrawerTruckPlate, todayStr, truck.trailer || '', 'Reassignment', `Reassigned to truck ${selectedDrawerTruckPlate} (took over from ${currentTruckDriver.name})`);
                        recordTruckAssignment(truck, driver.name, driver.id, truck.trailer || '', todayStr, 'Reassignment', `Driver reassigned from ${currentTruckDriver.name} to ${driver.name}`);
                        syncDriverIdWithTruck(driver);

                        currentTruckDriver.license_plate = "";
                        recordDriverTruckAssignment(currentTruckDriver, "", todayStr, '', 'Released', `Released from truck ${selectedDrawerTruckPlate}`);
                        syncDriverIdWithTruck(currentTruckDriver);
                        detailsLog = `Assigned ${driver.name} to ${selectedDrawerTruckPlate} (trailer: ${truck.trailer || '-'}, released ${currentTruckDriver.name}).`;
                    }
                } else {
                    driver.license_plate = selectedDrawerTruckPlate;
                    recordDriverTruckAssignment(driver, selectedDrawerTruckPlate, todayStr, truck.trailer || '', 'Reassignment', `Assigned to truck ${selectedDrawerTruckPlate} (was unassigned)`);
                    recordTruckAssignment(truck, driver.name, driver.id, truck.trailer || '', todayStr, 'Reassignment', `Assigned driver ${driver.name} (was unassigned)`);
                    syncDriverIdWithTruck(driver);
                    detailsLog = `Assigned ${driver.name} to ${selectedDrawerTruckPlate} (trailer: ${truck.trailer || '-'}, was unassigned).`;
                }
                ...
                logs.unshift({
                    ...
                    driverId: driver.id,
                    truckPlate: selectedDrawerTruckPlate,
            ...
            function showPage(page) {
                const validPages = ['dashboard', 'drivers', 'trucks', 'jobcards', 'violations', 'hscpolicy', 'orders', 'settings', 'reports', 'recyclebin'];
                ...
                if (page === 'dashboard') renderDashboard();
                    ...
                    const savedTruckSubTab = localStorage.getItem('fg3_truck_subtab') || activeTruckSubTab || 'trucks';
                    setTruckSubTab(savedTruckSubTab); 
                }
                else if (page === 'jobcards') { 
                    const savedJcTab = localStorage.getItem('fg3_jc_page_tab') || jcPageTab || 'pipeline';
                    renderJobCardsPage(); 
                    setJcPageTab(savedJcTab);
                }
                else if (page === 'violations') { 
                    const savedVioSubpage = localStorage.getItem('fg3_violation_subpage') || violationSubpage || 'violations';
                    populateViolationFilters(); 
                    renderViolationMetrics(); 
                    switchViolationSubpage(savedVioSubpage); 
                }
                else if (page === 'settings') renderSettings();
                else if (page === 'hscpolicy') {
                    const savedHscTab = localStorage.getItem('fg3_hsc_subpage') || hscActiveSubpage || 'rules';
                    switchHscSubpage(savedHscTab);
                }
            ...
            function updateSidebarBadges() {
                const expiredDocs = (trucks || []).reduce((s, t) => s + (t.documents || []).filter(d => docStatus(d) === 'expired').length, 0)
                    + (trailers || []).reduce((s, t) => s + (t.documents || []).filter(d => docStatus(d) === 'expired').length, 0);
                ...
                if (elSidebarStatus) {
                    elSidebarStatus.textContent = `${(drivers || []).length} drivers · ${(trucks || []).length} trucks · ${(trailers || []).length} trailers`;
                }
            }
            ...
            function renderDashboard() {
                ...
                const trailerDocExpired = (trailers || []).reduce((s, t) => s + (t.documents || []).filter(d => docStatus(d) === 'expired').length, 0);
                const trailerDocExpiring = (trailers || []).reduce((s, t) => s + (t.documents || []).filter(d => docStatus(d) === 'expiring').length, 0);
                ...
                const totalExpiredDocs = expiredDocs + driverDocExpired + trailerDocExpired;
                const totalExpiringDocs = expiringDocs + driverDocExpiring + trailerDocExpiring;
                ...
                trucks.forEach(t => {
                    (t.documents || []).forEach(d => {
                        ...
                        if (st === 'expired') alerts.push({ icon: '🔴', title: `${formatTruckLabel(t)}: ${d.type} EXPIRED`, desc: `Expired on ${d.expiryDate}`, action: `App.showPage('trucks');App.setTruckSubTab('trucks');App.openTruckModal(${t._idx})` });
                        else if (st === 'expiring') alerts.push({ icon: '🟡', title: `${formatTruckLabel(t)}: ${d.type} expiring`, desc: `Expires ${d.expiryDate}`, action: `App.showPage('trucks');App.setTruckSubTab('trucks');App.openTruckModal(${t._idx})` });
                    });
                });
                (trailers || []).forEach(tr => {
                    (tr.documents || []).forEach(d => {
                        const st = docStatus(d);
                        if (st === 'expired') alerts.push({ icon: '🔴', title: `Trailer ${tr.id || 'N/A'}: ${d.type} EXPIRED`, desc: `Expired on ${d.expiryDate}`, action: `App.showPage('trucks');App.setTruckSubTab('trailers');setTimeout(()=>App.openTrailerModal(${tr._idx}),100)` });
                        else if (st === 'expiring') alerts.push({ icon: '🟡', title: `Trailer ${tr.id || 'N/A'}: ${d.type} expiring`, desc: `Expires ${d.expiryDate}`, action: `App.showPage('trucks');App.setTruckSubTab('trailers');setTimeout(()=>App.openTrailerModal(${tr._idx}),100)` });
                    });
                });
                drivers.forEach(d => {
        ...
        `;
                ...
                const docSummaryItems = [
                    ...trucks.map(t => {
                        ...
                        return `<div style="display:flex;align-items:center;gap:10px;padding:8px 0;border-bottom:1px solid var(--border);cursor:pointer" onclick="App.showPage('trucks');App.setTruckSubTab('trucks');App.openTruckModal(${t._idx})">
                ...
                <span class="badge badge-${worst === 'expired' ? 'critical' : worst === 'expiring' ? 'warn' : 'good'}">${worst}</span></div>`;
                    ...
                    ...(trailers || []).map(t => {
                        const expired = (t.documents || []).filter(d => docStatus(d) === 'expired').length;
                        const expiring = (t.documents || []).filter(d => docStatus(d) === 'expiring').length;
                        const valid = (t.documents || []).filter(d => docStatus(d) === 'valid').length;
                        const worst = expired ? 'expired' : expiring ? 'expiring' : (t.documents || []).length > 0 ? 'valid' : 'missing';
                        const assignedTruck = getAssignedTruckForTrailer(t.id);
                        const truckInfo = assignedTruck ? ` (Attached to ${assignedTruck.plate})` : ' (Unattached)';
                        return `<div style="display:flex;align-items:center;gap:10px;padding:8px 0;border-bottom:1px solid var(--border);cursor:pointer" onclick="App.showPage('trucks');App.setTruckSubTab('trailers');setTimeout(()=>App.openTrailerModal(${t._idx}),100)">
                <span>🛞</span><div style="flex:1"><strong>Trailer ${xmlEscape(t.id || '—')}</strong>${xmlEscape(truckInfo)}<br><small>${valid} valid, ${expiring} expiring, ${expired} expired</small></div>
                <span class="badge badge-${worst === 'expired' ? 'critical' : worst === 'expiring' ? 'warn' : worst === 'valid' ? 'good' : 'badge-suspended'}">${worst}</span></div>`;
                    }),
                    ...drivers.map(d => {
        ...
        ` : '';
            ...
            <div class="modal-body">
                ...
                <div class="section">
                    <div class="section-title" style="display:flex;justify-content:space-between;align-items:center">
                        <span>Violations Summary (${d.violations.length})</span>
                        <div style="display:flex;gap:6px">
                            <button class="btn btn-primary btn-xs" onclick="App.showAddViolationForm(${idx})">+ Add</button>
                            ${d.violations.length > 0 ? `<button class="btn btn-ghost btn-xs" onclick="App.openViolationDetailModal(${idx})">View All (${d.violations.length}) →</button>` : ''}
                        </div>
                    </div>
                    ...
                    <div style="margin-top:10px;display:flex;flex-direction:column;gap:6px">
                        <div style="font-size:11px;font-weight:600;color:var(--text3);text-transform:uppercase;letter-spacing:0.05em">Recent Violations (Latest First)</div>
                        ${[...(d.violations || [])].map((v, originalIndex) => ({ ...v, originalIndex }))
                            ...
                            .slice(0, 3).map(v => {
                                ...
                                return `<div style="display:flex;align-items:center;justify-content:space-between;gap:8px;padding:6px 10px;background:var(--bg3);border-radius:6px;font-size:12px;border-left:3px solid ${sevColor};cursor:pointer" onclick="App.openViolationDetailModal(${idx})">
                                    <div style="flex:1;min-width:0;overflow:hidden;text-overflow:ellipsis;white-space:nowrap">
                                        <strong>${xmlEscape(v.type || 'Violation')}</strong>
                                        ${v.description ? `<span style="color:var(--text3);margin-left:6px">— ${xmlEscape(v.description)}</span>` : ''}
                                    </div>
                                    <div style="display:flex;align-items:center;gap:6px;flex-shrink:0">
                                        <span style="font-size:11px;color:var(--text3)">📅 ${xmlEscape(v.date || '—')}</span>
                                        <span class="vtag vtag-${v.severity || 'low'}" style="font-size:9px">${v.severity || 'low'}</span>
                                    </div>
                                </div>`;
                            }).join('')}
                    </div>` : ''}
            ...
            function changeDriverStatus(idx, newStatus) {
                ...
                if (isDriverUnassignStatus(newStatus) && prevTruckPlate) {
                    d.license_plate = '';
                    const todayStr = new Date().toISOString().slice(0, 10);
                    recordDriverTruckAssignment(d, '', todayStr, '', 'Released', `Status changed to ${newStatus}`);
                    const prevTrkObj = trucks.find(t => String(t.plate || '').trim().toUpperCase() === prevTruckPlate.toUpperCase());
                    if (prevTrkObj) {
                        recordTruckAssignment(prevTrkObj, 'Unassigned', '', prevTrkObj.trailer || '', todayStr, 'Driver Released', `Driver ${d.name} released due to status change (${newStatus})`);
                    }
                    ...
                    if (isDriverUnassignStatus(newStatus)) {
                        d.license_plate = '';
                        recordDriverTruckAssignment(d, '', new Date().toISOString().slice(0, 10), '', 'Released', `Status changed to ${newStatus}`);
                        syncDriverIdWithTruck(d);
                    }
                    saveAll();
                }
                    ...
                    <div style="flex:1;min-width:0">
                        <div style="display:flex;justify-content:space-between;gap:12px;align-items:flex-start">
                            <div style="min-width:0;">
                                ...
                                <div style="font-size:12px;font-weight:600;color:var(--amber);margin-top:2px">📅 ${xmlEscape(v.date || '—')}</div>
                            </div>
            ...
            function openViolationDetailModal(idx) {
                ...
                // Map all violations with originalIndex and sort by date descending (most recent first)
                ...
                const history = [...d.assignmentHistory].sort((a, b) => (b.date || '').localeCompare(a.date || ''));
                ...
                if (history.length === 0) {
                    ...
                    htmlContent = allV.length
                        ...
                        : '<p style="color:var(--text3);text-align:center;padding:16px">No violations recorded</p>';
                } else {
                    ...
                    history.forEach((assignItem, hIdx) => {
                        ...
                        // Sort matching violations by most recent date first
                        matching.sort((a, b) => (b.date || '').localeCompare(a.date || '') || (b.originalIndex - a.originalIndex));
                        ...
                        <div style="background:var(--bg3);border-left:4px solid var(--accent);border-radius:8px;padding:10px 14px;margin:${hIdx === 0 ? '0' : '16px'} 0 10px 0;font-size:12px;font-weight:700;color:var(--text);display:flex;align-items:center;justify-content:space-between">
                            <div style="display:flex;align-items:center;gap:8px">
                                ...
                                <span>Vehicle: <strong>${xmlEscape(truckPlate)}</strong> ${assignDate !== '—' && assignDate !== 'Initial' ? `<span style="font-size:11px;color:var(--text3);font-weight:400">(Reassigned on ${xmlEscape(assignDate)})</span>` : ''}</span>
                            </div>
            ...
            <div class="modal-body">
                <div style="font-size:11px;color:var(--text3);margin-bottom:12px;display:flex;align-items:center;gap:6px">
                    <span>ℹ️ Listed chronologically by most recent violation date first.</span>
                </div>
                ${htmlContent}
            </div>
            <div class="modal-actions">
            ...
            <div class="modal-body">
                ${d.accidentsList.length ? d.accidentsList.map((a, i) => `<div style="display:flex;flex-direction:column;gap:4px;padding:8px 0;border-bottom:1px solid var(--border)">
                    <div style="display:flex;justify-content:space-between;align-items:flex-start;gap:8px">
                        ...
                        <div style="display:flex;gap:4px;flex-shrink:0">
                            <button class="btn btn-xs btn-ghost btn-icon" style="opacity:0.45;transition:opacity 0.2s;font-size:13px" onmouseover="this.style.opacity=1" onmouseout="this.style.opacity=0.45" onclick="App.openEditAccidentModal(${idx}, ${i})" title="Edit record">✏️</button>
                            <button class="btn btn-xs btn-ghost btn-icon" style="opacity:0.45;transition:opacity 0.2s;font-size:13px;color:var(--red)" onmouseover="this.style.opacity=1" onmouseout="this.style.opacity=0.45" onclick="App.removeFromList(${idx},'accidentsList',${i})" title="Delete record">🗑️</button>
                        </div>
                    </div>
            ...
            function removeFromList(driverIdx, listName, itemIdx) {
                const d = drivers.find(x => x._idx === driverIdx);
                if (!d || !Array.isArray(d[listName]) || !d[listName][itemIdx]) return;
                const item = d[listName][itemIdx];
                if (listName === 'accidentsList') {
                    const recType = (item.recordType || item.type || 'accident').toLowerCase();
                    const typeLabel = recType === 'incident' ? 'Incident' : 'Accident';
                    const label = `${typeLabel}: ${d.name} (${item.date || 'No Date'})`;
                    sendToRecycleBin('accident', label, item, { driverIdx: d._idx, driverName: d.name });
                } else if (listName === 'violations') {
                    const label = `Violation: ${d.name} (${item.violationType || 'Violation'})`;
                    sendToRecycleBin('violation', label, item, { driverIdx: d._idx, driverName: d.name });
                }
                ...
                renderDriverCards(); renderDriverMetrics(); renderViolationMetrics(); updateSidebarBadges();
            }
            ...
            function setTruckSubTab(tab) {
                ...
                localStorage.setItem('fg3_truck_subtab', activeTruckSubTab);
            ...
            function renderTrailerMetrics() {
                ...
                const expiringDocs = trailers.reduce((s, t) => s + (t.documents || []).filter(d => docStatus(d) === 'expiring').length, 0);
            ...
            <div class="metric-card c-purple"><div class="metric-label">Expiring Docs</div><div class="metric-value">${expiringDocs}</div></div>`;
                }

                const alertBar = document.getElementById('trailerExpiryAlertBar');
                if (alertBar) {
                    if (expiredDocs > 0 || expiringDocs > 0) {
                        const alertParts = [];
                        if (expiredDocs > 0) alertParts.push(`<strong style="color:var(--red)">${expiredDocs} trailer document(s) EXPIRED</strong>`);
                        if (expiringDocs > 0) alertParts.push(`<strong style="color:var(--amber)">${expiringDocs} expiring within 30 days</strong>`);
                        alertBar.style.display = 'flex';
                        alertBar.innerHTML = `<span style="font-size:16px">${expiredDocs > 0 ? '🚨' : '⚠️'}</span> <div style="flex:1"><span>${alertParts.join(' &bull; ')} across your fleet trailers. Please review and renew required compliance documents.</span></div>`;
                    } else {
            ...
            function renderTrailerCards() {
                ...
                el.innerHTML = list.map(t => {
                    ...
                    let docAlertTag = '';
                    if (expired > 0) {
                        docAlertTag = `<div style="background:rgba(240,76,90,0.15);border:1px solid rgba(240,76,90,0.3);color:var(--red);padding:4px 8px;border-radius:6px;font-size:10px;font-weight:600;margin-top:6px;display:flex;align-items:center;gap:4px"><span>🔴</span> <span>${expired} Document${expired > 1 ? 's' : ''} EXPIRED</span></div>`;
                    } else if (expiring > 0) {
                        docAlertTag = `<div style="background:rgba(245,158,11,0.15);border:1px solid rgba(245,158,11,0.3);color:var(--amber);padding:4px 8px;border-radius:6px;font-size:10px;font-weight:600;margin-top:6px;display:flex;align-items:center;gap:4px"><span>⚠️</span> <span>${expiring} Document${expiring > 1 ? 's' : ''} expiring soon</span></div>`;
                    }
                    ...
                    ${docAlertTag || ((expired + expiring) > 0 ? `<div style="font-size:10px;color:var(--red);margin-top:4px">⚠ ${expired} expired, ${expiring} expiring</div>` : '')}
                </div>`;
                }).join('');
            }
            function openTrailerModal(idx) {
                ...
                let modalDocAlert = '';
                if (expired > 0) {
                    modalDocAlert = `<div style="background:rgba(240,76,90,0.15);border:1px solid rgba(240,76,90,0.3);color:var(--red);padding:10px 12px;border-radius:8px;margin-bottom:14px;font-size:12px;display:flex;align-items:center;gap:8px">
                        <span style="font-size:16px">🔴</span>
                        <div><strong>Document Alert:</strong> ${expired} document(s) expired. Please renew and update expiry date below.</div>
                    </div>`;
                } else if (expiring > 0) {
                    modalDocAlert = `<div style="background:rgba(245,158,11,0.15);border:1px solid rgba(245,158,11,0.3);color:var(--amber);padding:10px 12px;border-radius:8px;margin-bottom:14px;font-size:12px;display:flex;align-items:center;gap:8px">
                        <span style="font-size:16px">⚠️</span>
                        <div><strong>Document Warning:</strong> ${expiring} document(s) expiring within 30 days.</div>
                    </div>`;
                }
            ...
            function updateTruckTrailer(idx, newTrailer) {
                ...
                if (trimmed) {
                    ...
                    if (existing) {
                        ...
                        const curExtDrv = drivers.find(d => String(d.license_plate || '').trim().toUpperCase() === existing.plate.toUpperCase());
                        existing.trailer = '';
                        recordTruckAssignment(existing, curExtDrv ? curExtDrv.name : '', curExtDrv ? curExtDrv.id : '', '', new Date().toISOString().slice(0, 10), 'Trailer Released', `Trailer ${trimmed} reassigned to ${t.plate}`);
                        if (curExtDrv) {
                            recordDriverTruckAssignment(curExtDrv, existing.plate, new Date().toISOString().slice(0, 10), '', 'Trailer Released', `Trailer ${trimmed} reassigned to ${t.plate}`);
                        }
                    }
                }
                t.trailer = trimmed;
                const todayStr = new Date().toISOString().slice(0, 10);
                const curDrv = drivers.find(d => String(d.license_plate || '').trim().toUpperCase() === t.plate.toUpperCase());
                recordTruckAssignment(t, curDrv ? curDrv.name : '', curDrv ? curDrv.id : '', trimmed, todayStr, 'Trailer Assigned', `Trailer updated to ${trimmed || 'None'} (was ${oldTrailer || 'None'})`);
                if (curDrv) {
                    recordDriverTruckAssignment(curDrv, t.plate, todayStr, trimmed, 'Trailer Assigned', `Trailer updated to ${trimmed || 'None'}`);
                }
                saveAll();
                // refresh UI
                openTruckModal(idx);
            ...
            function unassignTrailer(idx) {
                ...
                const todayStr = new Date().toISOString().slice(0, 10);
                const curDrv = drivers.find(d => String(d.license_plate || '').trim().toUpperCase() === t.plate.toUpperCase());
                recordTruckAssignment(t, curDrv ? curDrv.name : '', curDrv ? curDrv.id : '', '', todayStr, 'Trailer Unassigned', `Trailer "${oldTrailer}" unassigned`);
                if (curDrv) {
                    recordDriverTruckAssignment(curDrv, t.plate, todayStr, '', 'Trailer Unassigned', `Trailer "${oldTrailer}" unassigned`);
                }
                saveAll();
                openTruckModal(idx);
                renderTruckCards();
            ...
            function renderViolationMetrics() {
                const total = drivers.reduce((s, d) => s + (Array.isArray(d.violations) ? d.violations.length : 0), 0);
                const high = drivers.reduce((s, d) => s + (Array.isArray(d.violations) ? d.violations.filter(v => (v.severity || 'low').toLowerCase() === 'high').length : 0), 0);
                let accidents = 0;
                let incidents = 0;
                drivers.forEach(d => {
                    if (Array.isArray(d.accidentsList)) {
                        d.accidentsList.forEach(a => {
                            const t = (a.recordType || a.type || 'accident').toLowerCase();
                            if (t === 'incident') incidents++;
                            else accidents++;
                        });
                    }
                });
                const container = document.getElementById('violationMetrics');
                if (!container) return;
                container.innerHTML = `
            <div class="metric-card c-blue"><div class="metric-label">Total Violations</div><div class="metric-value">${total}</div></div>
            <div class="metric-card c-amber"><div class="metric-label">High Severity</div><div class="metric-value">${high}</div></div>
            <div class="metric-card c-red"><div class="metric-label">Accidents</div><div class="metric-value">${accidents}</div></div>
            <div class="metric-card c-purple"><div class="metric-label">Incidents</div><div class="metric-value">${incidents}</div></div>`;
            }
            function switchViolationSubpage(subpage) {
                violationSubpage = subpage;
                localStorage.setItem('fg3_violation_subpage', violationSubpage);
            ...
            function captureAccidentFilterState() {
                ...
                const typeEl = document.getElementById('accidentTypeFilter');
                if (typeEl) accidentTypeFilter = typeEl.value || accidentTypeFilter;
            ...
            function handleAccidentTypeFilter(value) {
                accidentTypeFilter = value || 'all';
                renderAccidentsSubpage();
            }
            ...
            function renderAccidentsSubpage() {
                ...
                const typeFilterVal = (document.getElementById('accidentTypeFilter')?.value || accidentTypeFilter || 'all').toLowerCase();
                const range = getAccidentDateRange();
                const visibleList = list.filter(item => {
                    const text = `${item.driver?.name || ''} ${item.truckPlate || item.driver?.license_plate || ''} ${item.description || ''} ${item.recordType || item.type || ''}`.toLowerCase();
                    const matchesSearch = !search || text.includes(search);
                    const itemType = (item.recordType || item.type || 'accident').toLowerCase();
                    const matchesType = (typeFilterVal === 'all') || (itemType === typeFilterVal);
                ...
                host.innerHTML = `
                    <div style="display:flex;justify-content:space-between;align-items:center;gap:10px;margin-bottom:12px;flex-wrap:wrap">
                        ...
                        <button class="btn btn-primary btn-sm" onclick="App.openAddAccidentModal()">+ Add Record</button>
                    </div>
                    <div class="toolbar" style="margin-bottom:12px;flex-wrap:wrap;gap:10px">
                        ...
                        <select id="accidentTypeFilter" onchange="App.handleAccidentTypeFilter(this.value)" style="padding:8px 10px;border:1px solid var(--border);border-radius:var(--radius);background:var(--bg3);color:var(--text);font-size:12px">
                            <option value="all" ${accidentTypeFilter === 'all' ? 'selected' : ''}>All</option>
                            <option value="accident" ${accidentTypeFilter === 'accident' ? 'selected' : ''}>Accidents</option>
                            <option value="incident" ${accidentTypeFilter === 'incident' ? 'selected' : ''}>Incidents</option>
                        </select>
                    ...
                    <div style="display:grid;gap:12px">
                        ${visibleList.length ? visibleList.map(item => {
                            const itemType = (item.recordType || item.type || 'accident').toLowerCase();
                            const isIncident = itemType === 'incident';
                            return `
                            <div class="accident-card" style="background:var(--bg2);border:1px solid var(--border);border-radius:14px;padding:14px;display:flex;flex-direction:column;gap:10px">
                                <div style="display:flex;justify-content:space-between;gap:10px;flex-wrap:wrap;align-items:flex-start">
                                    <div style="display:flex;align-items:center;gap:10px">
                                        <span class="badge ${isIncident ? 'badge-trip' : 'badge-suspended'}" style="font-size:10px;text-transform:uppercase;font-weight:700;padding:4px 8px">
                                            ${isIncident ? 'INCIDENT' : 'ACCIDENT'}
                                        </span>
                                        <div>
                                            <div class="accident-badge" style="font-size:11px;color:var(--text3);text-transform:uppercase;letter-spacing:0.04em">Driver</div>
                                            <div style="font-size:15px;font-weight:700;color:var(--text)">${xmlEscape(String(item.driver?.name || 'Unknown'))}</div>
                                        </div>
                                    </div>
                                    <div>
                                        <div class="accident-badge" style="font-size:11px;color:var(--text3);text-transform:uppercase;letter-spacing:0.04em">Truck Plate</div>
                                    ...
                                    <div style="text-align:right">
                                        <div class="accident-badge" style="font-size:11px;color:var(--text3);text-transform:uppercase;letter-spacing:0.04em">Date</div>
                                ...
                                <div style="display:flex;justify-content:flex-end;gap:8px">
                                    <button class="btn btn-ghost btn-xs btn-icon" style="opacity:0.45;transition:opacity 0.2s;font-size:14px;padding:4px 8px;color:var(--text2)" onmouseover="this.style.opacity=1" onmouseout="this.style.opacity=0.45" onclick="App.openEditAccidentModal(${item.driverIdx}, ${item.accidentIdx})" title="Edit record">✏️</button>
                                    <button class="btn btn-ghost btn-xs btn-icon" style="opacity:0.45;transition:opacity 0.2s;font-size:14px;padding:4px 8px;color:var(--red)" onmouseover="this.style.opacity=1" onmouseout="this.style.opacity=0.45" onclick="App.deleteAccidentEntry(${item.driverIdx}, ${item.accidentIdx})" title="Delete record">🗑️</button>
                                </div>
                            </div>`;
                        }).join('') : '<div class="empty-state"><div class="e-icon">🚗</div><p>No records match the current filters.</p></div>'}
                    </div>`;
            }
            ...
            function openAddAccidentModal() {
                ...
                openModal(`
                <div class="modal-header"><h3>Add Record</h3><div class="modal-close" onclick="App.closeModal()">✕</div></div>
                <div class="modal-body" style="display:flex;flex-direction:column;gap:10px">
                    <label style="font-weight:700;color:var(--accent)">Type *</label>
                    <select id="addAccidentRecordType" style="width:100%;padding:10px;border:1px solid var(--border2);border-radius:10px;background:var(--bg3);color:var(--text);font-weight:600">
                    ...
                    <textarea id="addAccidentDescription" rows="4" placeholder="Describe the accident or incident" style="width:100%;padding:10px;border:1px solid var(--border2);border-radius:10px;background:var(--bg3);color:var(--text)"></textarea>
                    <label>Measures Taken</label>
                    <textarea id="addAccidentMeasures" rows="3" placeholder="Measures taken after the event" style="width:100%;padding:10px;border:1px solid var(--border2);border-radius:10px;background:var(--bg3);color:var(--text)"></textarea>
                    ...
                    <div style="font-size:11px;color:var(--text3)">Add files for images or PDFs, or add video clips for the record. Videos must be 60 seconds or less.</div>
                    <div id="accidentSelectedFiles" style="font-size:12px;color:var(--text2)"></div>
                    <button id="saveAccidentButton" class="btn btn-primary btn-sm" onclick="App.saveAccidentFromForm()">Save Record</button>
                </div>`);
            }
            ...
            async function saveAccidentFromForm() {
                ...
                const recordType = document.getElementById('addAccidentRecordType')?.value || 'accident';
                ...
                if (!description) {
                    if (saveButton) { saveButton.disabled = false; saveButton.textContent = 'Save Record'; }
                    return showToast('Please add a description.');
                }
            ...
            function deleteAccidentEntry(driverIdx, accidentIdx) {
                const d = drivers.find(x => x._idx === driverIdx);
                if (!d || !Array.isArray(d.accidentsList) || !d.accidentsList[accidentIdx]) return;
                const removedRecord = d.accidentsList[accidentIdx];
                const recType = (removedRecord.recordType || removedRecord.type || 'accident').toLowerCase();
                const typeLabel = recType === 'incident' ? 'Incident' : 'Accident';
                if (!confirm(`Delete this ${typeLabel.toLowerCase()} record? It will be moved to the Recycle Bin.`)) return;

                const label = `${typeLabel}: ${d.name} (${removedRecord.date || 'No Date'})`;
                sendToRecycleBin('accident', label, removedRecord, { driverIdx: d._idx, driverName: d.name });
                ...
                renderAccidentsSubpage();
                updateSidebarBadges();
                showToast(`${typeLabel} moved to Recycle Bin`);
            }
            ...
            function openEditAccidentModal(driverIdx, accidentIdx) {
                ...
                const currentType = (accident.recordType || accident.type || 'accident').toLowerCase();

                openModal(`
                <div class="modal-header"><h3>Edit Record</h3><div class="modal-close" onclick="App.closeModal()">✕</div></div>
                <div class="modal-body" style="display:flex;flex-direction:column;gap:10px">
                    <label style="font-weight:700;color:var(--accent)">Type *</label>
                    <select id="editAccidentRecordType" style="width:100%;padding:10px;border:1px solid var(--border2);border-radius:10px;background:var(--bg3);color:var(--text);font-weight:600">
                        <option value="accident" ${currentType === 'accident' ? 'selected' : ''}>Accident</option>
                    ...
                    <label>Description</label>
                    <textarea id="editAccidentDescription" rows="4" placeholder="Describe the accident or incident" style="width:100%;padding:10px;border:1px solid var(--border2);border-radius:10px;background:var(--bg3);color:var(--text)">${xmlEscape(accident.description || '')}</textarea>
                    <label>Measures Taken</label>
                    <textarea id="editAccidentMeasures" rows="3" placeholder="Measures taken after the event" style="width:100%;padding:10px;border:1px solid var(--border2);border-radius:10px;background:var(--bg3);color:var(--text)">${xmlEscape(accident.measuresTaken || '')}</textarea>
            ...
            async function saveEditAccidentForm(originalDriverIdx, accidentIdx) {
                ...
                const recordType = document.getElementById('editAccidentRecordType')?.value || 'accident';
            ...
            function buildTruckPickerHtml(pickerId, excludePlates) {
                ...
                const rows = availTrucks.map(t => {
                    ...
                    const drv = getDriverForTruckPlate(t.plate);
                    const drvName = drv && drv.name ? drv.name : '';
                    const searchText = `${t.plate || ''} ${t.brand || ''} ${t.model || ''} ${t.chassisNo || ''} ${drvName}`.toLowerCase();
                    return `<label class="tp-row" data-plate="${t.plate.toLowerCase()}" data-driver="${drvName.toLowerCase()}" data-search="${searchText}" style="display:flex;align-items:center;gap:8px;padding:8px 10px;background:var(--bg4);border:1px solid var(--border);border-radius:6px;cursor:${inMaint ? 'not-allowed' : 'pointer'};opacity:${inMaint ? '0.45' : '1'};transition:all 0.15s"
                                ...
                                <div style="display:flex;flex-direction:column;gap:1px;min-width:0;flex:1">
                                    <span style="font-size:13px;font-weight:600;color:var(--text);white-space:nowrap">${formatTruckLabel(t)}${t.model ? ` · <span style="font-size:11px;font-weight:400;color:var(--text3)">${xmlEscape(t.model)}</span>` : ''}</span>
                                    ${drvName ? `<span style="font-size:11px;color:var(--text2);display:flex;align-items:center;gap:4px">👤 Driver: <strong style="color:var(--accent)">${xmlEscape(drvName)}</strong></span>` : '<span style="font-size:10px;color:var(--text3);font-style:italic">No driver currently assigned</span>'}
                                </div>
                ...
                return `<div style="display:flex;flex-direction:column;gap:8px" id="truckPicker_${pickerId}">
                    <div style="position:relative">
                        <input type="text" id="tpInput_${pickerId}" placeholder="Type truck plate or driver name (e.g. RAD801E or Damien)…"
                            ...
                            onfocus="this.style.borderColor='var(--accent)'" onblur="setTimeout(() => { const s = document.getElementById('tpSuggestions_${pickerId}'); if(s) s.style.display='none'; }, 250); this.style.borderColor='var(--border)'">
                        <span style="position:absolute;left:10px;top:50%;transform:translateY(-50%);font-size:14px;pointer-events:none">🔍</span>
                    </div>
                    <div id="tpSuggestions_${pickerId}" style="display:none;flex-direction:column;gap:4px;background:var(--bg2);border:1px solid var(--accent);border-radius:8px;padding:6px;max-height:140px;overflow-y:auto;box-shadow:0 4px 14px rgba(0,0,0,0.35);z-index:20"></div>
            ...
            function selectPickerTruck(pickerId, plate) {
                const cb = document.querySelector(`.tpCb_${pickerId}[value="${plate}"]`);
                if (cb && !cb.disabled) {
                    cb.checked = true;
                }
                syncPickerFromCheckbox(pickerId);
                const sugg = document.getElementById(`tpSuggestions_${pickerId}`);
            ...
            function syncPickerFromInput(pickerId) {
                ...
                const availPlates = Array.from(cbs).map(cb => cb.value.toUpperCase());

                // Check if any token matches a driver name or plate
                let hasConvertedDriver = false;
                const mappedPlates = [];

                tokens.forEach(token => {
                    const tUpper = token.toUpperCase();
                    if (availPlates.includes(tUpper)) {
                        mappedPlates.push(tUpper);
                    } else {
                        // Look for driver match in the fleet
                        const drvMatch = (drivers || []).find(d => 
                            String(d.name || '').trim().toUpperCase() === tUpper && d.license_plate
                        );
                        if (drvMatch && drvMatch.license_plate) {
                            const plateUpper = drvMatch.license_plate.toUpperCase();
                            if (availPlates.includes(plateUpper)) {
                                mappedPlates.push(plateUpper);
                                hasConvertedDriver = true;
                            }
                        }
                    }
                });

                // If user typed a comma after a driver's name, or ended typing, replace with truck plate in input
                if (hasConvertedDriver && rawVal.includes(',')) {
                    const uniquePlates = [...new Set(mappedPlates)];
                    input.value = uniquePlates.length ? `${uniquePlates.join(', ')}, ` : '';
                }

                // Update checkboxes
                cbs.forEach(cb => {
                    if (!cb.disabled) {
                        cb.checked = mappedPlates.includes(cb.value.toUpperCase());
                    }
                });
                ...
                rows.forEach(row => {
                    ...
                    const shouldShow = !lastToken || searchText.includes(lastToken);
                    row.style.display = shouldShow ? 'flex' : 'none';
                    if (shouldShow) matchCount++;
                });

                // Populate interactive instant suggestions if searching
                const suggContainer = document.getElementById(`tpSuggestions_${pickerId}`);
                if (suggContainer) {
                    if (lastToken && lastToken.length >= 2) {
                        // Find matching trucks or drivers
                        const matches = trucks.filter(t => {
                            const drv = getDriverForTruckPlate(t.plate);
                            const drvName = drv && drv.name ? drv.name.toLowerCase() : '';
                            const plate = (t.plate || '').toLowerCase();
                            const brand = (t.brand || '').toLowerCase();
                            return plate.includes(lastToken) || drvName.includes(lastToken) || brand.includes(lastToken);
                        }).slice(0, 5);

                        if (matches.length) {
                            suggContainer.innerHTML = matches.map(t => {
                                const drv = getDriverForTruckPlate(t.plate);
                                const drvHtml = drv && drv.name ? ` · 👤 <strong style="color:var(--accent)">${xmlEscape(drv.name)}</strong>` : '';
                                return `<div onclick="App.selectPickerTruck('${pickerId}', '${t.plate}')" style="display:flex;align-items:center;justify-content:space-between;gap:8px;padding:6px 10px;background:var(--bg3);border-radius:6px;font-size:12px;cursor:pointer;transition:background 0.15s"
                                             onmouseenter="this.style.background='var(--bg4)'" onmouseleave="this.style.background='var(--bg3)'">
                                    <div>
                                        <span style="font-weight:700;color:var(--text)">🚛 ${formatTruckLabel(t)}</span>
                                        <span style="font-size:11px;color:var(--text2)">${drvHtml}</span>
                                    </div>
                                    <span style="font-size:10px;color:var(--green);font-weight:600">Click to Select Plate ➔</span>
                                </div>`;
                            }).join('');
            ...
            function getPickerPlates(pickerId) {
                ...
                typed.forEach(token => {
                    const tUpper = token.toUpperCase();
                    if (allKnownPlates.includes(tUpper)) {
                        const match = trucks.find(t => t.plate.toUpperCase() === tUpper);
                        if (match) resolvedTyped.push(match.plate);
                    } else {
                        // Check if it matches a driver name (exact or partial)
                        const drv = (drivers || []).find(d => 
                            String(d.name || '').trim().toUpperCase() === tUpper ||
                            String(d.name || '').trim().toUpperCase().includes(tUpper)
                        );
                        if (drv && drv.license_plate) {
                            const trkMatch = trucks.find(t => t.plate.toUpperCase() === drv.license_plate.toUpperCase());
                            if (trkMatch) resolvedTyped.push(trkMatch.plate);
                        }
                    }
                });

                const merged = [...new Set([...cbPlates, ...resolvedTyped])];
                return merged;
            }
            ...
            function openOrderDetailsModal(idx) {
                ...
                if (o.assignedTrucks.length === 0) {
                    ...
                    const buildRow = (t, isActive) => {
                        ...
                        // Calculate transit days for current leg
                        ...
                        // Multi-trip: archived trips + current active trip
                        const archivedTrips = Array.isArray(t.trips) ? t.trips : [];
                        const totalTripCount = archivedTrips.length + 1;
                        const isMultiTrip = archivedTrips.length > 0;
                        const tripHistoryId = `tripHistory_${idx}_${t._tIdx}`;

                        // Build expandable trip history
                        let tripHistoryHtml = '';
                        if (isMultiTrip) {
                            const allTrips = [
                                ...archivedTrips,
                                {
                                    tripNumber: totalTripCount,
                            ...
                            tripHistoryHtml = `
                            <div id="${tripHistoryId}" style="display:none;margin-top:10px;border-top:1px solid var(--border);padding-top:10px">
                                <div style="font-size:10px;font-weight:700;color:var(--text3);text-transform:uppercase;letter-spacing:0.05em;margin-bottom:8px">Trip History</div>
                                ${allTrips.map(trip => {
                                    const tDays = trip.transitDays || (() => {
                                        if (trip.loadedDate && trip.offloadDate) {
                                            const d = Math.ceil((new Date(trip.offloadDate) - new Date(trip.loadedDate)) / 86400000);
                                            return `${d} day${d !== 1 ? 's' : ''}`;
                                        }
                                        return '—';
                                    })();
                                    return `<div style="display:flex;gap:6px;align-items:center;padding:6px 8px;background:${trip.isCurrent ? 'rgba(61,127,255,0.08)' : 'rgba(255,255,255,0.03)'};border-radius:6px;margin-bottom:4px;border-left:2px solid ${trip.isCurrent ? 'var(--accent)' : 'var(--border)'}">
                                        <div style="font-size:10px;font-weight:700;color:${trip.isCurrent ? 'var(--accent)' : 'var(--text3)'};min-width:42px;flex-shrink:0">
                                            Trip ${trip.tripNumber}${trip.isCurrent ? ' 🟢' : ''}
                                        </div>
                                        <div style="display:grid;grid-template-columns:repeat(4,1fr);gap:4px;flex:1;font-size:9px">
                                            <div style="background:rgba(255,255,255,0.05);padding:4px 6px;border-radius:4px">
                                                <div style="color:var(--text3);margin-bottom:1px">Allocated</div>
                                                <div style="color:var(--text2);font-weight:600">${trip.allocatedDate || '—'}</div>
                                            </div>
                                            <div style="background:rgba(255,255,255,0.05);padding:4px 6px;border-radius:4px">
                                                <div style="color:var(--text3);margin-bottom:1px">Loaded</div>
                                                <div style="color:var(--text2);font-weight:600">${trip.loadedDate || '—'}</div>
                                            </div>
                                            <div style="background:rgba(255,255,255,0.05);padding:4px 6px;border-radius:4px">
                                                <div style="color:var(--text3);margin-bottom:1px">Offloaded</div>
                                                <div style="color:var(--text2);font-weight:600">${trip.offloadDate || '—'}</div>
                                            </div>
                                            <div style="background:rgba(255,255,255,0.05);padding:4px 6px;border-radius:4px">
                                                <div style="color:var(--text3);margin-bottom:1px">In Transit</div>
                                                <div style="color:var(--accent);font-weight:600">${tDays}</div>
                                            </div>
                                        </div>
                                    </div>`;
                                }).join('')}
                        ...
                        // Multi-trip badge: shown above status dropdown on right side
                        const multiTripBadge = isMultiTrip && isActive
                            ? `<button onclick="(function(){var el=document.getElementById('${tripHistoryId}');if(el)el.style.display=el.style.display==='none'?'block':'none';})()" style="display:flex;align-items:center;gap:4px;padding:3px 8px;background:rgba(255,165,0,0.12);border:1px solid rgba(255,165,0,0.35);border-radius:12px;color:var(--amber);font-size:10px;font-weight:700;cursor:pointer;margin-bottom:6px;width:fit-content">
                                🔁 ×${totalTripCount} trips <span style="font-size:9px;opacity:0.7">▼</span>
                              </button>`
                            : '';
                        ...
                        return `<div style="${!isActive ? 'opacity:0.45;' : ''}display:flex;align-items:stretch;gap:12px;padding:12px;background:var(--bg4);border-left:3px solid ${isActive ? sColor : 'var(--border)'};border-radius:8px;transition:all 0.15s"
                            ...
                            <div style="flex-shrink:0;display:flex;flex-direction:column;align-items:flex-end;justify-content:flex-start;gap:6px;min-width:120px">
                                ${multiTripBadge}
                                ${selectHtml}
                            </div>
                        </div>`;
                            ...
                            <div class="section">
                                <div class="section-title" style="display:flex;justify-content:space-between;align-items:center;margin-bottom:10px">
                                    ...
                                    <div style="display:flex;gap:6px">
                                        ...
                                        <button class="btn btn-ghost btn-xs" onclick="App.reassignTruckForm(${idx})" style="color:var(--amber);border-color:rgba(255,165,0,0.35)">🔄 Reassign Truck</button>
                                    </div>
                                </div>
                                ...
                                <div id="reassignTruckContainer_${idx}" style="display:none;margin-bottom:12px;padding:14px;background:var(--bg3);border:1px solid rgba(255,165,0,0.4);border-radius:10px">
                                    <div style="display:flex;align-items:center;gap:6px;margin-bottom:10px">
                                        <span style="font-size:14px">🔄</span>
                                        <span style="font-size:12px;font-weight:700;color:var(--text)">Reassign Truck (New Trip)</span>
                                    </div>
                                    <p style="font-size:11px;color:var(--text3);margin-bottom:10px">Select an <strong>Offloaded</strong> truck to reassign it to this order for another trip. Its previous trip data will be archived and it will return to <em>Allocated</em> status.</p>
                                    <div id="reassignTruckSlot_${idx}">
                                        ${(() => {
                                            const offloadedTrucks = o.assignedTrucks.map((t, i) => ({...t, _tIdx: i})).filter(t => t.active !== false && t.status === 'Offloaded');
                                            if (!offloadedTrucks.length) {
                                                return `<div style="font-size:12px;color:var(--text3);font-style:italic;padding:8px 0">No offloaded trucks available for reassignment. A truck must be <strong>Offloaded</strong> before it can be reassigned.</div>`;
                                            }
                                            return offloadedTrucks.map(t => `
                                                <div style="display:flex;align-items:center;justify-content:space-between;gap:10px;padding:8px 10px;background:var(--bg4);border-radius:6px;margin-bottom:6px">
                                                    <div>
                                                        <div style="font-weight:700;font-size:12px">${formatTruckLabelFromPlate(t.plate)}</div>
                                                        <div style="font-size:10px;color:var(--text3)">Offloaded on ${t.offloadDate || '—'}</div>
                                                    </div>
                                                    <button class="btn btn-primary btn-xs" onclick="App.confirmReassignTruck(${idx}, ${t._tIdx})">🔄 Reassign</button>
                                                </div>`).join('');
                                        })()}
                                    </div>
                                    <div style="margin-top:10px">
                                        <button class="btn btn-ghost btn-xs" onclick="document.getElementById('reassignTruckContainer_${idx}').style.display='none'">Cancel</button>
                                    </div>
                                </div>
            ...
            function addTruckToOrderForm(idx) {
                ...
                const reassignC = document.getElementById(`reassignTruckContainer_${idx}`);
                if (reassignC) reassignC.style.display = 'none';
            ...
            function reassignTruckForm(idx) {
                const o = orders.find(x => x._idx === idx);
                if (!o) return;

                const container = document.getElementById(`reassignTruckContainer_${idx}`);
                if (!container) return;
            ...
            function confirmReassignTruck(orderIdx, truckIdx) {
                ...
                // Only allow reassignment of Offloaded trucks
                if (assignment.status !== 'Offloaded') {
                    showToast('⚠ Only Offloaded trucks can be reassigned for a new trip');
                    return;
                }

                // Ensure trips array exists
                if (!Array.isArray(assignment.trips)) assignment.trips = [];

                // Calculate transit days for the completed trip
                let tripTransitDays = '—';
                if (assignment.loadedDate && assignment.offloadDate) {
                    const d = Math.ceil((new Date(assignment.offloadDate) - new Date(assignment.loadedDate)) / 86400000);
                    tripTransitDays = `${d} day${d !== 1 ? 's' : ''}`;
                }

                // Archive the current trip
                const completedTripNumber = assignment.trips.length + 1;
                assignment.trips.push({
                    tripNumber: completedTripNumber,
                    allocatedDate: assignment.allocatedDate || o.date || null,
                    loadedDate: assignment.loadedDate || null,
                    transitDate: assignment.transitDate || null,
                    offloadDate: assignment.offloadDate || null,
                    transitDays: tripTransitDays
                });

                // Reset the current trip fields for the new leg
                const todayDate = new Date().toISOString().slice(0, 10);
                assignment.status = 'allocated';
                assignment.allocatedDate = todayDate;
                assignment.loadedDate = null;
                assignment.transitDate = null;
                assignment.offloadDate = null;
                assignment.multiTrip = true;

                // Release the truck from On Trip if needed (it was Offloaded, so already Active, but ensure)
                ...
                renderOrders();
                showToast(`✓ ${assignment.plate} reassigned — Trip ${completedTripNumber + 1} started (Allocated)`);
            }
            ...
            function confirmSwitchTruck(idx) {
                ...
                const fromInput = document.getElementById(`switchTruckFrom_${idx}`);
                const toInput = document.getElementById(`switchTruckTo_${idx}`);
                let fromPlate = fromInput?.value.trim().toUpperCase();
                let toPlate = toInput?.value.trim().toUpperCase();
                ...
                // Resolve driver name to truck plate if needed
                const drvFrom = (drivers || []).find(d => String(d.name || '').trim().toUpperCase() === fromPlate || String(d.name || '').trim().toUpperCase().includes(fromPlate));
                if (!trucks.some(t => t.plate.toUpperCase() === fromPlate) && drvFrom && drvFrom.license_plate) {
                    fromPlate = drvFrom.license_plate.toUpperCase();
                }

                const drvTo = (drivers || []).find(d => String(d.name || '').trim().toUpperCase() === toPlate || String(d.name || '').trim().toUpperCase().includes(toPlate));
                if (!trucks.some(t => t.plate.toUpperCase() === toPlate) && drvTo && drvTo.license_plate) {
                    toPlate = drvTo.license_plate.toUpperCase();
                }
            ...
            function updateOrderTruckStatus(orderIdx, truckIdx, newStatus) {
                ...
                if (newStatus === 'Transit' && !assignment.transitDate) assignment.transitDate = todayDate;
            ...
            function switchHscSubpage(subpage) {
                ...
                localStorage.setItem('fg3_hsc_subpage', hscActiveSubpage);
            ...
            function isSameCategoryAttachment(existingFile, category) {
                if (!existingFile) return false;
                const catLower = String(category || '').trim().toLowerCase();
                if (!catLower) return false;

                const fCat = String(existingFile.category || '').trim().toLowerCase();
                const fName = String(existingFile.name || '').trim().toLowerCase();
                const fDocType = String(existingFile.docType || existingFile.type || '').trim().toLowerCase();

                // 1. Direct category match
                if (fCat && fCat === catLower) return true;
                // 2. Direct docType match
                if (fDocType && fDocType === catLower) return true;
                // 3. Name contains category
                if (fName && fName.includes(catLower)) return true;

                // 4. Category-specific keywords/aliases
                if (catLower === 'insurance' && (fName.includes('insur') || fCat.includes('insur'))) return true;
                if (catLower === 'comesa' && (fName.includes('comesa') || fCat.includes('comesa'))) return true;
                if (catLower === 'calibration' && (fName.includes('calib') || fCat.includes('calib'))) return true;
                if (catLower === 'yellow card' && (fName.includes('yellow') || fCat.includes('yellow') || fName.includes('card'))) return true;
                if (catLower === 'inspection' && (fName.includes('inspect') || fCat.includes('inspect'))) return true;
            ...
            function extractDateFromFileName(fileName) {
                ...
                m = fileName.match(/\b(0[1-9]|[12]\d|3[01])[-_\./](0[1-9]|1[0-2])[-_\./](20\d{2})\b/);
            ...
            <div class="modal-body">
                <div style="display:flex;flex-direction:column;gap:14px">
                    ...
                    <select id="bulkAttachmentTargetDestination">
                        ...
                        <option value="driver">Drivers Subpage</option>
                    </select>
            ...
            async function processBulkAttachmentImport() {
                ...
                const entities = destination === 'trailer' ? trailers : destination === 'driver' ? drivers : trucks;
                const plateField = destination === 'trailer' ? 'id' : destination === 'driver' ? 'driver_id' : 'plate';
                const plateMap = (entities || []).map(item => {
                    const keys = [];
                    if (destination === 'truck') {
                        if (item.plate) keys.push(normalizePlate(item.plate));
                    } else if (destination === 'trailer') {
                        if (item.id) keys.push(normalizePlate(item.id));
                    } else if (destination === 'driver') {
                        if (item.driver_id) keys.push(normalizePlate(item.driver_id));
                        if (item.name) keys.push(normalizePlate(item.name));
                        if (item.license_plate) keys.push(normalizePlate(item.license_plate));
                    }
                    return { item, keys: keys.filter(Boolean) };
                ...
                for (let index = 0; index < files.length; index += 1) {
                    ...
                    if (normalizedFileName) {
                        const matches = plateMap.filter(p => p.keys.some(k => normalizedFileName.includes(k)));
                        if (matches.length === 1) {
                            match = matches[0].item;
                        } else if (matches.length > 1) {
                            match = matches.reduce((best, current) => {
                                const bestMax = Math.max(...best.keys.map(k => k.length));
                                const currentMax = Math.max(...current.keys.map(k => k.length));
                    ...
                    const displayPlate = normalizePlate(match[plateField]) || String(match[plateField] || match.name || '').toUpperCase();
                    const record = {
                        id: fileId,
                    ...
                    // 1. Immediately replace any existing attachment matching the document category
                    if (!Array.isArray(match.files)) match.files = [];
                    const beforeCount = match.files.length;
                    match.files = match.files.filter(f => !isSameCategoryAttachment(f, category));
                    const numReplaced = beforeCount - match.files.length;
                    if (numReplaced > 0) {
                        summary.replaced += numReplaced;
                    }
                    match.files.push(record);

                    // 2. Synchronize / update document expiry date in match.documents (for trucks / trailers)
                    const extractedDate = extractDateFromFileName(file.name);
                    const defaultNextYear = new Date(Date.now() + 365 * 86400000).toISOString().slice(0, 10);
                    const catLower = category.toLowerCase().trim();

                    if (destination === 'truck' || destination === 'trailer') {
                        if (!Array.isArray(match.documents)) match.documents = [];
                        const docIdx = match.documents.findIndex(d => (d.type || '').trim().toLowerCase() === catLower);
                        if (docIdx >= 0) {
                            const currentDoc = match.documents[docIdx];
                            const isExpired = typeof docStatus === 'function' ? ['expired', 'expiring'].includes(docStatus(currentDoc)) : false;
                            const newExpiry = extractedDate || (isExpired ? defaultNextYear : currentDoc.expiryDate || defaultNextYear);
                            match.documents[docIdx].expiryDate = newExpiry;
                            summary.docsUpdated += 1;
                        } else if (category !== 'Other') {
                            match.documents.push({
                                type: category,
                                expiryDate: extractedDate || defaultNextYear
                            });
                            summary.docsUpdated += 1;
                        }
                    } else if (destination === 'driver') {
                        if (catLower.includes('license')) {
                            match.licenseExpiry = extractedDate || defaultNextYear;
                            summary.docsUpdated += 1;
                        } else if (catLower.includes('passport')) {
                            match.passportExpiry = extractedDate || defaultNextYear;
                            summary.docsUpdated += 1;
                        }
                    }
            ...
            function getDriverForTruckPlate(plate) {
                if (!plate) return null;
                const normalized = String(plate || '').trim().toUpperCase();
                return (drivers || []).find(d => String(d.license_plate || '').trim().toUpperCase() === normalized) || null;
            }

            function formatTruckPickerLabel(truck) {
                if (!truck) return '';
                const base = formatTruckLabel(truck);
                const modelPart = truck.model ? ` · ${xmlEscape(truck.model)}` : '';
                const drv = getDriverForTruckPlate(truck.plate);
                const drvPart = drv && drv.name ? ` · 👤 ${xmlEscape(drv.name)}` : '';
                return `${base}${modelPart}${drvPart}`;
            }
...
${sheets.map(s => sheetXml(s.name, s.rows)).join('')}
            ...
            function renderReports() {
                ...
                const savedRptCat = localStorage.getItem('fg3_reports_cat');
                const activeCatTab = document.querySelector('#page-reports .rpt-tabs .rpt-tab[data-cat].active');
                const catToShow = savedRptCat || (activeCatTab ? activeCatTab.dataset.cat : 'driver');
                switchReportCategory(catToShow);
            }

            function switchReportCategory(cat) {
                localStorage.setItem('fg3_reports_cat', cat);
            ...
            function buildDriverReportHtml(targetDrivers, types, dateFrom, dateTo) {
                ...
                targetDrivers.forEach(drv => {
                    ...
                    if (types.includes('reassignments') || types.includes('driver-reassignments')) {
                        let history = [...(drv.assignmentHistory || [])];
                        if (history.length === 0 && drv.license_plate) {
                            const trObj = trucks.find(t => String(t.plate || '').trim().toUpperCase() === String(drv.license_plate || '').trim().toUpperCase());
                            history.push({
                                truckPlate: drv.license_plate,
                                trailer: (drv.custom && drv.custom.Trailer) || (trObj ? trObj.trailer : '') || '',
                                action: 'Initial Assignment',
                                date: drv.hire_date || 'Initial',
                                notes: 'Initial vehicle assignment'
                            });
                        }
                        history.sort((a, b) => new Date(b.date || b.timestamp || 0) - new Date(a.date || a.timestamp || 0));
                        const filteredHistory = _filterByDate(history, dateFrom, dateTo);
                        html += `<div class="rpt-block"><div class="rpt-block-title">🔁 Reassignment History (${filteredHistory.length})</div>
                        ${filteredHistory.length ? `<table class="rpt-table"><thead><tr><th style="width:120px">Reassignment Date</th><th>Assigned Truck</th><th>Assigned Trailer</th><th>Action</th><th>Details &amp; Notes</th></tr></thead><tbody>
                        ${filteredHistory.map(h => {
                            const dStr = h.date || (h.timestamp ? h.timestamp.slice(0, 10) : '—');
                            const trk = h.truckPlate || 'Unassigned';
                            const trl = h.trailer || (h.custom && h.custom.Trailer) || '—';
                            const act = h.action || (trk === 'Unassigned' ? 'Released' : 'Reassigned');
                            const chipCls = act.toLowerCase().includes('swap') ? 'warning' : act.toLowerCase().includes('unassign') || act.toLowerCase().includes('release') ? 'expired' : 'valid';
                            const notes = h.notes || (h.prevTruckPlate ? `Previous truck: ${h.prevTruckPlate}` : '—');
                            return `<tr>
                                <td><strong>${xmlEscape(dStr)}</strong></td>
                                <td><strong>${xmlEscape(trk)}</strong></td>
                                <td>${xmlEscape(trl)}</td>
                                <td><span class="rpt-chip rpt-chip-${chipCls}">${xmlEscape(act.toUpperCase())}</span></td>
                                <td style="font-size:11px;color:var(--text2)">${xmlEscape(notes)}</td>
                            </tr>`;
                        }).join('')}
                        </tbody></table>` : '<div class="rpt-empty">No reassignment history in selected period</div>'}</div>`;
                    }
            ...
            function buildTruckReportHtml(targetTrucks, types, dateFrom, dateTo) {
                ...
                targetTrucks.forEach(trk => {
                    ...
                    if (types.includes('truck-reassignments')) {
                        let history = [...(trk.assignmentHistory || [])];
                        (drivers || []).forEach(d => {
                            (d.assignmentHistory || []).forEach(dh => {
                                if (String(dh.truckPlate || '').trim().toUpperCase() === String(trk.plate || '').trim().toUpperCase()) {
                                    const exists = history.some(th => (th.date === dh.date || (th.timestamp && dh.timestamp && th.timestamp === dh.timestamp)) && (th.driverName === d.name || th.driverId === d.id));
                                    if (!exists) {
                                        history.push({
                                            date: dh.date,
                                            ...
                                            driverName: d.name,
                                            driverId: d.id,
                                            trailer: dh.trailer || trk.trailer || '',
                                            action: dh.action || 'Driver Reassigned',
                                            notes: dh.notes || `Driver ${d.name} assigned to ${trk.plate}`
                                        });
                                    }
                                }
                            });
                        ...
                        if (history.length === 0) {
                            const curD = drivers.find(d => String(d.license_plate || '').trim().toUpperCase() === trk.plate.toUpperCase());
                            if (curD || trk.trailer) {
                                history.push({
                                    driverName: curD ? curD.name : 'Unassigned',
                                    driverId: curD ? curD.id : '',
                                    trailer: trk.trailer || '',
                                    action: 'Initial Assignment',
                                    date: (curD && curD.hire_date) || 'Initial',
                                    notes: 'Initial truck assignment recorded'
                                });
                            }
                        }
                        ...
                        const filteredHistory = _filterByDate(history, dateFrom, dateTo);
                        html += `<div class="rpt-block"><div class="rpt-block-title">🔁 Reassignment History (${filteredHistory.length})</div>
                        ${filteredHistory.length ? `<table class="rpt-table"><thead><tr><th style="width:120px">Reassignment Date</th><th>Assigned Driver</th><th>Assigned Trailer</th><th>Action</th><th>Details &amp; Notes</th></tr></thead><tbody>
                        ${filteredHistory.map(h => {
                            const dStr = h.date || (h.timestamp ? h.timestamp.slice(0, 10) : '—');
                            const drvStr = h.driverName ? `${h.driverName}${h.driverId ? ` (#${h.driverId})` : ''}` : 'Unassigned';
                            const trl = h.trailer || trk.trailer || '—';
                            const act = h.action || (h.driverName && h.driverName !== 'Unassigned' ? 'Driver Reassigned' : 'Released');
                            const chipCls = act.toLowerCase().includes('swap') ? 'warning' : act.toLowerCase().includes('unassign') || act.toLowerCase().includes('release') ? 'expired' : 'valid';
                            const notes = h.notes || '—';
                            return `<tr>
                                <td><strong>${xmlEscape(dStr)}</strong></td>
                                <td><strong>${xmlEscape(drvStr)}</strong></td>
                                <td>${xmlEscape(trl)}</td>
                                <td><span class="rpt-chip rpt-chip-${chipCls}">${xmlEscape(act.toUpperCase())}</span></td>
                                <td style="font-size:11px;color:var(--text2)">${xmlEscape(notes)}</td>
                            </tr>`;
            ...
            function init() {
                ...
                loadAll(function() {
                    ...
                    const savedPage = localStorage.getItem('fg3_active_page');
                    const initialPage = validPages.includes(hashPage) ? hashPage : (validPages.includes(savedPage) ? savedPage : 'dashboard');
                    showPage(initialPage);

                    window.addEventListener('hashchange', () => {
                        const p = (window.location.hash || '').replace('#', '').trim();
                        if (validPages.includes(p) && p !== getActivePage()) {
                            showPage(p);
                        }
                    });
                });
            ...
            function setJcPageTab(tab) {
                jcPageTab = tab;
                localStorage.setItem('fg3_jc_page_tab', jcPageTab);
  ...
  <section class="full-section">
    ...
    ${proofSection}
            ...
            return {
                ...
                handleAccidentPeriodFilter, handleAccidentTypeFilter, handleAccidentDateFilter,
                ...
                reassignTruckForm, confirmReassignTruck,
                addTruckToOrderForm, confirmAddTruck,
                syncPickerFromInput, syncPickerFromCheckbox, selectPickerTruck, updatePickerCount, getPickerPlates, buildTruckPickerHtml, toggleTruckPickerRow,
                getDriverForTruckPlate,
